...
module Lotus
  module Commands
    class Action
      # @since 0.3.0
      # @api private
      class New < ::Lotus::Generators::Abstract
        # @since 0.3.0
        # @api private
        ACTION_SEPARATOR = /\/|\#/
        ...
        def assert_existing_app!
          unless target.join(app_root).exist?
            raise Lotus::Commands::Action::Error.new("Unknown app: `#{ app_name }'")
          end
        end
        ...
        def assert_action!
          if @action.nil?
            raise Lotus::Commands::Action::Error.new("Unknown action, please add action's name with this syntax controller_name#action_name")
          end
        end
