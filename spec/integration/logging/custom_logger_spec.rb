...
RSpec.describe "Logging / Configuring a custom logger", :app_integration do
  include Rack::Test::Methods

  describe "configured via config.logger=" do
    let(:app) { Hanami.app }
  ...
  describe "replacing the provider with a manually registered logger" do
    # The logger is decorated to guarantee Hanami's structured and tagged logging interface, so a
    # replaced logger is wrapped with Hanami::UniversalLogger (with the original logger underneath).
    specify "wraps the custom logger with Hanami::UniversalLogger (on booted container)" do
      with_tmp_directory(Dir.mktmpdir) do
        write_custom_logger_provider

        require "hanami/setup"
        Hanami.boot

        expect(Hanami.app[:logger]).to be_a(Hanami::UniversalLogger)
        expect(Hanami.app[:logger].logger).to eq "custom logger"
      end
    end

    specify "wraps the custom logger with Hanami::UniversalLogger (resolved lazily, on non-booted container)" do
      with_tmp_directory(Dir.mktmpdir) do
        write_custom_logger_provider
    ...
    def write_custom_logger_provider
      write "config/app.rb", <<~RUBY
        require "hanami"
